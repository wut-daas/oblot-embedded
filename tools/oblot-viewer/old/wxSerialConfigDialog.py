...
class SerialConfigDialog(wx.Dialog):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.initial_baudrate = None
        if 'baudrate' in kwds:
            self.initial_baudrate = kwds['baudrate']
        del kwds['baudrate']
    ...
    def __set_properties(self):
        ...
        if self.show & SHOW_BAUDRATE:
            ...
            preferred_baudrate = self.serial.baudrate
            if self.initial_baudrate is not None:
                preferred_baudrate = self.initial_baudrate

            for n, baudrate in enumerate(self.serial.BAUDRATES):
                self.combo_box_baudrate.Append(str(baudrate))
                if preferred_baudrate == baudrate:
                    preferred_index = n
